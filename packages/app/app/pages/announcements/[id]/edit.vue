...
async function submit() {
    ...
    try {
        ...
        setTimeout(() => router.push(`/announcements/${announcementDocumentId}`), 800)
    } catch (err) {
...
<template>
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div v-else-if="errorMessage"
            ...
            <NuxtLink to="/announcements" class="mt-4 text-sm text-primary-600 hover:underline">{{ t.backToAnnouncements
                }}</NuxtLink>
        </div>
        ...
        <template v-else>
            ...
            <div class="flex items-center gap-4 transition-all duration-500"
                ...
                <NuxtLink :to="`/announcements/${announcementDocumentId}`"

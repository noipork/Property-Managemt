...
const { subscription, fetchSubscription, daysRemaining, isExpired, isPending } = useSubscription()
...
const { redirectToCheckout, verifyCheckoutSuccess, isLoading: isPaymentLoading, error: paymentError } = useStripe()
                ...
                <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div v-for="plan in plans" :key="plan.id"
                            ...
                            <div v-else>
                                ...
                                <div class="flex items-baseline gap-1">
                                    <span class="text-3xl font-bold text-gray-900 dark:text-white">
                                        {{ formatPrice(getDiscountedPrice(plan.price, selectedDuration), plan.currency)
                                        }}
                                    </span>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                    ≈ {{ formatPrice(getMonthlyEquivalent(plan.price, selectedDuration), plan.currency)
                                    }}/{{ t.month }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="mt-auto pt-4">
                            <button v-if="activePlan?.id !== plan.id" @click="handleSelectPlan(plan)"
                                ...
                                <span v-else>
                                    {{ plan.price === 0 ? t.getStarted : t.upgradePlan }}
                                </span>
                            </button>
                            <div v-else
        ...
        <Teleport to="body">
            <Transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0"
                ...
                <div v-if="showErrorModal"
                    ...
                    <div class="w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden">
                        <!-- Error Header -->
                        <div class="relative bg-gradient-to-br from-red-500 to-rose-500 px-6 py-8 text-center">
                            <div class="relative">
                                <div
                                    class="w-20 h-20 mx-auto mb-4 bg-white/20 backdrop-blur rounded-full flex items-center justify-center">
                        ...
                        <div class="px-6 py-6 text-center">
                            ...
                            <button @click="closeErrorModal"
                                class="w-full py-3 px-4 bg-gray-600 hover:bg-gray-700 text-white font-semibold rounded-xl transition-colors">
                                {{ t.tryAgain }}
                            </button>
                        </div>
                    </div>

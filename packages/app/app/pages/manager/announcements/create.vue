...
async function fetchProperties() {
    try {
        ...
        if (propertiesList.value.length > 0) {
            form.value.propertyId = propertiesList.value[0].documentId
        }
    } catch { /* ignore */ }
}
...
function handleAdditionalImagesSelect(e: Event) {
    const input = e.target as HTMLInputElement
    if (input.files) {
        const files = Array.from(input.files).slice(0, 10 - additionalImages.value.length)
        files.forEach(file => {
...
async function submit() {
    ...
    try {
        ...
        if (additionalImages.value.length > 0) {
            ...
            const uploadedFiles = await uploadRes.json()
            additionalImageIds = uploadedFiles.map((f: any) => f.id)
        }
        ...
        if (coverImageId) {
            body.data.coverImage = coverImageId
        }
        if (additionalImageIds.length > 0) {
            body.data.images = additionalImageIds
        }

        const res = await fetch(`${STRAPI_URL}/api/announcements`, {
            method: 'POST',
            headers: {
        ...
        const data = await res.json()
        showToast('success', t.value.announcementCreated)
        setTimeout(() => router.push(`/manager/announcements/${data.data.documentId}`), 800)
    } catch (err) {
        showToast('error', t.value.announcementCreateError)
    } finally {
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4 transition-all duration-500"
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ t.createAnnouncementSubtitle }}</p>
            </div>
        </div>

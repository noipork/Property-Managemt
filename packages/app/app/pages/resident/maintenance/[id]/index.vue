...
const canCancel = computed(() => request.value?.status === 'pending')
...
async function sendMessage() {
    ...
    try {
        ...
        // Only refetch if socket is disconnected; real-time event handles it otherwise
...
async function cancelRequest() {
    ...
    try {
        ...
        showToast('success', t.value.maintenanceCancelled || 'Request cancelled')
        await fetchRequest()
    } catch {
        showToast('error', t.value.maintenanceCancelError || 'Failed to cancel request')
    } finally {
...
onMounted(async () => {
    ...
    cleanupFunctions.push(
        onMaintenanceUpdated((data) => {
            if (data.maintenanceDocumentId === maintenanceDocumentId && request.value) {
                // Refetch to get the fully populated updated record
                fetchRequest()
            }
        })
    )
...
<template>
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <div v-else-if="errorMessage"
            ...
            <NuxtLink to="/resident/maintenance" class="mt-4 text-sm text-primary-600 hover:underline">{{
                t.backToMaintenance }}
            </NuxtLink>
        </div>
        ...
        <template v-else-if="request">
            ...
            <div class="flex items-start justify-between gap-4 transition-all duration-500"
                ...
                <div v-if="canCancel" class="shrink-0">
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">
                                    {{ t.cancelMaintenanceConfirm || 'Are you sure you want to cancel request' }}
                                    <strong class="text-gray-900 dark:text-white">{{ request?.requestNumber }}</strong>?
                                    {{ t.cancelMaintenanceConfirm2 || 'This action cannot be undone.' }}
                                </p>
                            </div>
                        </div>
                        <div class="flex justify-end gap-3 mt-6">
                            <button @click="showCancelModal = false" :disabled="isCancelling"

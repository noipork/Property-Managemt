...
export default factories.createCoreController('api::lease.lease', ({ strapi }) => ({
  ...
  async update(ctx) {
    ...
    const oldRecords = await strapi.documents('api::lease.lease').findMany({
      ...
      populate: {
        resident: { fields: ['documentId', 'username'] },
        property: { fields: ['documentId', 'name'], populate: { owner: { fields: ['documentId', 'username'] } } },
      },
    });
    const oldLease = oldRecords?.[0] as any;
    ...
    if (response?.data && oldLease) {
      ...
      if (oldLease.status !== newLease.status) {
        try {
          const statusLabels: Record<string, string> = {
            pending: 'Pending',
          ...
          const leaseNo = newLease.leaseNo || oldLease.leaseNo;
          const propertyName = oldLease.property?.name;
          const residentName = oldLease.resident?.username || 'Resident';

          // ── Resident → reviewing: notify manager-role users of this property ─
          if (newLease.status === 'reviewing' && oldLease.property?.documentId) {
            const managers = await strapi.documents('plugin::users-permissions.user').findMany({
              filters: {
                property: { documentId: { $eq: oldLease.property.documentId } },
                $or: [
                  { role: { id: { $eq: 3 } } },
                  { role: { type: { $eq: 'manager' } } },
                ],
              },
              fields: ['documentId'],
            });
            ...
            const managerRecipientIds = managers
              .map((manager: any) => manager?.documentId)
              .filter((id: unknown): id is string => typeof id === 'string' && id.length > 0)
              .filter((id: string) => id !== user.documentId);

            if (managerRecipientIds.length > 0) {
              await strapi.service('api::notification.notification').createAndNotifyMany({
                title: `Lease submitted for review: ${leaseNo}`,
                message: `${residentName} has submitted their lease information${propertyName ? ` at ${propertyName}` : ''}. Please review and approve.`,
                type: 'lease',
                priority: 'high',
                relatedDocumentId: newLease.documentId,
                actionUrl: `/manager/leases/${newLease.documentId}`,
                metadata: {
                  leaseNo,
          ...
          // ── All other status changes: notify the resident ──────────────────
          if (newLease.status !== 'reviewing' && oldLease.resident?.documentId) {
            const statusMessages: Record<string, string> = {
              pending: 'Your lease has been returned to pending status. Please check the details and resubmit.',
              active: 'Your lease has been approved and is now active. Welcome!',
              expired: 'Your lease has expired.',
              terminated: 'Your lease has been terminated.',
              cancelled: 'Your lease has been cancelled.',
            };

            const message = statusMessages[newLease.status]
              || `Lease status changed from ${statusLabels[oldLease.status] || oldLease.status} to ${statusLabels[newLease.status] || newLease.status}`;

            await strapi.service('api::notification.notification').createAndNotify({
              title: `Lease status updated: ${statusLabels[newLease.status] || newLease.status}`,
              message,
              type: 'lease',
          ...
          strapi.log.error('[Notification] Failed to send lease status notification:', err);
        }
      }
